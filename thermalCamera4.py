# read from Arduino IR thermal sensor array & display
# uses the new MySerial class; swap two buffers 
...
data = np.array([randint(0,10) for i in range(ROWS*COLS)])
    ...
    try:
        if (previous_flag == use_first_buffer):  # no new data available
            ...
            if (use_first_buffer):
                ...
                img.set_array(buffer1) 
                fig.canvas.draw()
            else:
                print ('second buffer has new data')
                img.set_array(buffer2) 
                fig.canvas.draw()
